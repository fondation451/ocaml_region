...
let to_poly mty = T.TPoly(StrSet.elements (fv_mty mty), mty)
...
let rec type_infer env t =
  ...
  |S.Var(var) -> begin
    try
      subs_empty, T.mk_term (T.Var(var)) (to_poly (instanciate (StrMap.find var env)))
    with Not_found -> raise (T.Type_Error "Type_infer")
  end
  |S.Unit -> subs_empty, T.mk_term T.Unit (to_poly T.TUnit)
  |S.Bool(b) -> subs_empty, T.mk_term (T.Bool(b)) (to_poly T.TBool)
  |S.Int(i) -> subs_empty, T.mk_term (T.Int(i)) (to_poly T.TInt)
  |S.Binop(op, t1, t2) ->
    ...
    compose_subs s1 (compose_subs s2 (compose_subs s3 s4)), T.mk_term (T.Binop(op, t1', t2')) (to_poly T.TInt)
  |S.Not(t1) ->
    let s1, t1' = type_infer env t1 in
    let mty1 = mty_of (T.get_type t1') in
    let s2 = mgu (apply_m s1 mty1) T.TBool in
    compose_subs s1 s2, T.mk_term (T.Not(t1')) (to_poly T.TBool)
  |S.Neg(t1) ->
    let s1, t1' = type_infer env t1 in
    let mty1 = mty_of (T.get_type t1') in
    let s2 = mgu (apply_m s1 mty1) T.TInt in
    compose_subs s1 s2, T.mk_term (T.Neg(t1')) (to_poly T.TInt)
  |S.Comp(c, t1, t2) ->
    let s1, t1' = type_infer env t1 in
    let s2, t2' = type_infer (apply_env s1 env) t2 in
    let mty1 = mty_of (T.get_type t1') in
    ...
    compose_subs s1 (compose_subs s2 (compose_subs s3 s4)), T.mk_term (T.Comp(c, t1', t2')) (to_poly T.TBool)
  |S.Fun(x_l, t1, t2) ->
    ...
    s, T.mk_term (T.Fun(x_l, t1', t2')) (to_poly (T.TFun(List.map (fun mty -> apply_m s mty) a_l, mty1, "DUMB REGION")))
  |S.App(t1, t2_l) ->
    ...
    compose_subs s3 (compose_subs s2 s1), T.mk_term (T.App(t1', t2_l')) (to_poly (apply_m s3 mty))
  |S.If(t1, t2, t3) ->
    let s1, t1' = type_infer env t1 in
    let mty1 = mty_of (T.get_type t1') in
    ...
    compose_subs s1' (compose_subs s2 (compose_subs s3 s4)), T.mk_term (T.If(t1', t2', t3')) (to_poly (apply_m s4 mty2))
  |S.Let(x, t1, t2) ->
    let s1, t1' = type_infer env t1 in
    let mty1 = mty_of (T.get_type t1') in
  ...
  |S.Newrgn -> subs_empty, T.mk_term T.Newrgn (to_poly (T.THnd(mk_rgn ())))
  |S.Aliasrgn(t1, t2) -> assert false
  |S.Freergn(t1) -> assert false

let type_inference env t =
  let s, t = type_infer env t in
  T.mk_term (T.get_term t) (generalize env (apply_m s (mty_of (T.get_type t))))
