    ...
    | Add l_l ->
      ...
      let rec loop l_l out =
        ...
        | l::l_l' -> begin
          ...
          | Some coef -> begin
            match l with
            | Var x when StrSet.mem x coef -> begin
...
let canonic_form_coef env coef_opt lit =
  let work lit = lit
(*    |> (fun lit -> Printf.printf "||||> IN %s\n" (show lit); lit)*)
    |> apply env
(*    |> (fun lit -> Printf.printf "apply %s\n" (show lit); lit)*)
...
let canonic_coef coef_opt lit = canonic_form_coef StrMap.empty coef_opt lit
...
let div_lit l_lit l =
  let rec loop l_lit out =
    ...
    | l'::l_lit' -> begin
      match l, l' with
      | Lit i, Lit i' -> Some (List.rev_append l_lit' out, Lit (i/i'))
      | _ ->
        if equal l' l then
          Some (List.rev_append l_lit' out, Unit)
        else
          loop l_lit' (l'::out)
    end
  in loop l_lit []

let rec div_l l_lit l_l =
  let rec loop l_lit l_l out =
    match l_l with
    | [] -> l_lit, out
    | l::l_l' -> begin
      match div_lit l_lit l with
      | None -> loop l_lit l_l' (l::out)
      | Some (l_lit', Unit) -> loop l_lit' l_l' out
      | Some (l_lit', l') -> loop l_lit' l_l' (l'::out)
    end
  in loop l_lit l_l []

let div l_lit l =
  match canonic l with
  | Mul l_l ->
    let l_lit', l_l' = div_l l_lit l_l in
    l_lit', (canonic (Mul l_l'))
  | _ as l -> begin
    match div_lit l_lit l with
    | None -> l_lit, l
    | Some (l_lit', l') -> l_lit', l'
  end

let resolve_0 lit coef =
(*  Printf.printf "Resolve DE %s\n\n" (show lit);*)
  let canonic = canonic_coef (Some coef) in
  let rec loop l old out =
    let out = canonic out in
(*    Printf.printf "%s\n?=\n%s\n\n\n" (show l) (show out);*)
    if equal l old then
      None
    else
      ...
      | Var x -> begin
        match out with
        | Lit out_i -> Some (x, out_i)
        | _ -> None
      end
      | Lit i -> raise Bad_equation
      | Add l_l ->
        let (l_l', l_out) = List.partition (fun l -> StrSet.exists (fun v -> is_in v l) coef) l_l in
        loop (canonic (Add l_l')) l (Add [out ; Mul [Lit (-1) ; Add l_out]])
      | Mul l_l -> begin
        let (l_l', l_out) = List.partition (fun l -> StrSet.exists (fun v -> is_in v l) coef) l_l in
        let l_out', out' = div l_out out in
        loop (canonic (Mul (List.rev_append l_out' l_l'))) l out'
      end
      | RShift l -> loop l lit (Mul [Lit 2 ; out])
      | Unit -> raise Bad_equation
  in loop (canonic lit) Unit (Lit 0)
