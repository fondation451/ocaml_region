...
let rec merge_mty mty_out mty_checked =
  Printf.printf "MERGE MTY\n%s\n%s\n\n" (S.show_rcaml_type mty_out) (T.show_rcaml_type mty_checked);
...
(* Construction CIN *)
let rec rgn_of t env =
  let merge_rgn _ _ _ = Some(T.Relaxed) in
  match S.get_term t with
  |S.Unit |S.Bool(_) |S.Int(_) |S.Newrgn |S.Nil |S.Leaf -> StrMap.empty
  | S.Var v -> begin
    try
      let T.TFun (_, _, _, _, cin, _, _) = merge_mty (S.get_type t) (StrMap.find v env) in
      List.fold_left (fun out (r, c) -> StrMap.add r c out) StrMap.empty cin
  ...
  |S.Assign(t1, t2) |S.Aliasrgn(t1, t2) |S.Sequence(t1, t2) ->
      StrMap.union merge_rgn (rgn_of t1 env) (rgn_of t2 env)
  |S.If(t1, t2, t3) ->
      StrMap.union merge_rgn (rgn_of t1 env) (StrMap.union merge_rgn (rgn_of t2 env) (rgn_of t3 env))
  |S.MatchList(var_match, t2, _, _, t3) |S.MatchTree(var_match, t2, _, _, _, t3) ->
      StrMap.union merge_rgn (rgn_of t2 env) (rgn_of t3 env)
  |S.App(t1, arg_l) -> rgn_of t1 env
...
let S.TPoly(a_l, r_l, ty) = S.get_type t in *)
  ...
  (*      Printf.printf "TYPE %s\n\n\n" (T.show_rcaml_type mty'); *)
      ...
      |S.Fun(f, arg_l, t1, t2, pot), S.TFun(mty_l, mty1, r) ->
          ...
          env arg_l mty_l
            in
      let cin = T.cap_of_strmap (rgn_of t1 env) in
      (* print_cap cin "cin"; *)
...
(*      let t_l', g2, c2, phi2 =
      ...
      |T.TFun(s, arg_mty_l, mty_res, mty_r, cin, cout, phie) as f_mty ->
          Printf.printf "TUTUTUTTUTURTUA %s\n\n" (T.show_rcaml_type f_mty);
        let c1 = T.cap_map (fun (r, c) -> (try List.assoc r s with Not_found -> r), c) c1 in
         print_cap c1 "c1";
        print_cap cin "cin";
        print_cap cout "cout";
(*        assert false;*)
